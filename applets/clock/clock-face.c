                                ...
                                gint      *natural_width)
{
    ...
    if (priv->size_widget != NULL) {
            ...
            *minimal_width = *natural_width = gdk_pixbuf_get_width (CDK_PIXBUF (priv->face_pixbuf));
    } else {
                                 ...
                                 gint      *natural_height)
{
    ...
    if (priv->size_widget != NULL) {
            ...
            *natural_height = child_natural_height + child_natural_height / 8;
            ...
            /* Use the size of the current pixbuf */
            *minimal_height = *natural_height = gdk_pixbuf_get_height (CDK_PIXBUF (priv->face_pixbuf));
    } else {
            /* we don't know anything, so use known dimensions for the svg
             * files */
            if (priv->size == CLOCK_FACE_LARGE)
